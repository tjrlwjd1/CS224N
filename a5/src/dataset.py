...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_len = int(torch.randint(4, int(self.block_size * 7 / 8) + 1, (1,))[0])
        truncated_doc = document[:truncated_len]

        masked_content_len = int(torch.randint(1, 2 * int(truncated_len / 4), (1,))[0])
        masked_content_index = int(
            torch.randint(0, truncated_len - int(truncated_len / 4) + 1, (1,))[0]
        )

        prefix = truncated_doc[:masked_content_index]
        suffix = truncated_doc[masked_content_index + masked_content_len :]
        masked_content = truncated_doc[
            masked_content_index : masked_content_index + masked_content_len
        ]
